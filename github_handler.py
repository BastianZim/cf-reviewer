...
def _get_sorted_prs(config):
    ...
    try:
        ...
        command = ["gh", "pr", "list", "-R", repo_name, "-s", "open", "--json", "number,title,url,createdAt"] + label_args.split()
        ...
        print(f"Error getting sorted PRs: {e}")
        return []
...
def _get_meta_yaml_files(pr_number, repo_name):
    ...
    try:
        command = ["gh", "pr", "diff", str(pr_number), "-R", repo_name, "--name-only"]
        result = subprocess.run(command, capture_output=True, text=True, check=True)
        ...
        print(f"Error getting meta.yaml files: {e}")
        return []
...
def _download_meta_yaml_content(pr_number, repo_name, meta_yaml_file, token):
    ...
    try:
        ...
        print(f"Error downloading meta.yaml content: {e}")
        return ""
